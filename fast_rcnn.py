...
class Fast_RCNN(nn.Module):
    ...
    def forward(self, feature_map, proposal_data):
        ...
        if self.training:
            ...
            spp_output = self.sppnet(feature_map, proposals_bbox)
            ...
            loss, loss_sc, bbox_regression_loss = self.calc_loss( roi_scores, roi_predicted_bbox,
                                                        proposals_labels, proposal_data['proposals_reference'] )
            
        
       

            boxes = self.box_coder.decode( roi_predicted_bbox.detach()[:,1]   , [ torch.cat(proposals_bbox) ] ).view( len(proposals_bbox), -1, 4)
    ...
    def calc_loss(self, roi_scores, roi_predicted_bbox, proposals_labels, proposals_reference):
                                                  
        loss_list, loss_sc_list, bbox_regression_list = [], [], []
      

        stacked_labels = torch.cat(proposals_labels)
        ...
        loss_targeted_box_contri = torch.cat(proposals_reference)
